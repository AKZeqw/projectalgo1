...
def lakukan_pembelian(username):
    try:
        ...
        print("Daftar Produk:")
        print(produk)
        produk_id = input("Masukkan ID Produk yang ingin dibeli: ")
        if produk_id in produk['id'].astype(str).values:
            ...
            if konfirmasi.lower() == 'y':
                transaksi_baru = pd.DataFrame([[username, produk_terpilih['nama'].values[0], jumlah, total_harga, dt.datetime.now()]], 
                                              columns=['username', 'produk', 'jumlah', 'total_harga', 'tanggal'])
                try:
